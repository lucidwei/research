WITH IndexClassification AS (
	...
	WHEN fund_fullname SIMILAR TO '%(中盘|中证500|国证1000)%' THEN '中盘'
	WHEN fund_fullname SIMILAR TO '%(小盘|国证2000|中证1000|中证2000|科创100)%' THEN '小盘'
    ...
    WHERE product_type = 'fund'
      AND fund_fullname NOT SIMILAR TO '%(货币|债)%'
